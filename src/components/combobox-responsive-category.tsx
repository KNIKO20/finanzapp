...
export function ComboBoxResponsiveCategory({ onChange, value }: Props) {
      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button variant="outline" className="w-[170px] justify-start text-neutral-50 overflow-x-auto overflow-y-hidden">
            {selectedStatus ? <>{selectedStatus.label}</> : value ? <>{value[0].toUpperCase() + value.slice(1)}</> : <>+ Set Category</>}
          </Button>
        </PopoverTrigger>
